...
var TotalRows = 10; //The total number of rows on the game board.
var TotalCols = 10; //The total number of columns on the game board.
...
function init() {
    ...
    audioTag.src = "sounds/start-new.wav";
    audioTag.play();
}
/*
...
function createTable() {
    ...
    for (colNumber = 0; colNumber < TotalCols; colNumber++) {
        ...
        for (rowNumber = 0; rowNumber < TotalRows; rowNumber++) {
            ...
            imgTag.setAttribute("data-row",rowNumber);
            imgTag.setAttribute("data-col",colNumber);
...
function handle_click(event) {
    ...
    var col = parseInt(document.getElementById(event.currentTarget.id).attributes['data-col'].value);
    var row = parseInt(document.getElementById(event.currentTarget.id).attributes['data-row'].value);
    ...
    if (highLightedImgs.length > 1 && gameBoard[col][row].style.border == borderOn) {
        destroyBubbles();
        audioTag.src = "sounds/bubble-pop.wav";
        audioTag.play();
    }
    else {
...
function destroyBubbles() {
    ...
    for (i = 0; i < highLightedImgs.length; i++) {
        ...
        var col = parseInt(document.getElementById(highLightedImgs[i].id).attributes['data-col'].value);
        colBubblesLeft[col] -= 1; //decrement bubbles left in the given column
        console.log(colBubblesLeft)
    }
    ...
    if (!movesLeft()) {
        ...
        document.getElementById('alertbox').innerHTML = "No more moves left! Game Over! \n Your Score was <b>"+currentScore+"</b>\n<button class='newgame' id='resetGame'>Restart</button>";
...
function expand(col, row, img) {
    _expand(col - 1, row, img,'left'); //left
    _expand(col + 1, row, img,'right'); //right
    _expand(col, row - 1, img,'up'); //up
    _expand(col, row + 1, img,'down'); //down
}

function _expand(col, row, img,dir) {
    //console.log("expand check ::::"+col,row+"===="+dir)
    ...
    && gameBoard[col][row].style.border != borderOn) {
      ...
      //  console.log("starting from matched=======")
        expand(col, row, img);
    }
}
/*
